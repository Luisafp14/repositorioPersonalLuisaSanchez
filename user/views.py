...
def ver_servicios(request):
    ...
    if resultados:
        ...
        servicios = servicios.filter(category__in=resultados)
    ...
    return render(request, 'ver_servicios.html', {'servicios': servicios, 'carrito_detalles': carrito_detalles, 'cantidad_en_carrito': cantidad_en_carrito})
...
def generar_factura(request):
    ...
    if request.method == 'POST':
        ...
        if factura_form.is_valid():
            ...
            # Verifica si 'carrito' está presente en la sesión antes de intentar eliminarlo
            if 'carrito' in request.session:
                del request.session['carrito']

            # Redirige a la nueva vista que mostrará la factura generada
            return redirect('factura_generada', factura_id=nueva_factura.id)

    else:
        factura_form = FacturaForm()

    return render(request, 'ver_servicios.html', {'servicios_adquiridos': servicios_adquiridos, 'factura_form': factura_form})

def factura_generada(request, factura_id):
    factura = get_object_or_404(Factura, id=factura_id)
    return render(request, 'factura_generada.html', {'factura': factura})